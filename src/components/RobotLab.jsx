...
const RobotLab = () => {
  ...
  const [inventory, setInventory] = useState(() => {
    const saved = typeof window !== 'undefined' && localStorage.getItem('robotParts');
    return saved ? JSON.parse(saved) : ALL_PARTS.slice(0, 10); // start with 10 parts
  });
            ...
            <div className="flex items-center mb-4">
              ...
              <div className="flex-1 h-4 bg-gray-700 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${playerHealth / robots[activeBot].stats.health * 100}%` }}
                />
              </div>
              <span className="ml-3 text-red-400">{playerHealth} / {robots[activeBot].stats.health}</span>
            </div>
            <button
              onClick={upgradeBot}
